...
if ($method === 'GET') {
    ...
    if ($action === 'load') {
        ...
        try {
            // KPIs
            $stmtKPI = $pdo->prepare("SELECT total_gross, total_deductions, total_net FROM v_period_totals WHERE period_label = ?");
            $stmtKPI->execute([$period]);
            $kpis = $stmtKPI->fetch(PDO::FETCH_ASSOC) ?: ['total_gross' => 0, 'total_deductions' => 0, 'total_net' => 0];

            // Total OT Pay
            $stmtOT = $pdo->prepare("SELECT SUM(ot_pay) as total_ot FROM payroll WHERE period_label = ?");
            ...
            $stmtDept = $pdo->prepare("
                ...
                WHERE pr.period_label = ? GROUP BY d.department_name ORDER BY net_total DESC
            ");
            ...
            $stmtGovt = $pdo->prepare("
                ...
                WHERE dt.is_mandatory = 1 AND d.payroll_period = ?
            ...
            $stmtTop = $pdo->prepare("
                ...
                FROM v_payroll_summary WHERE period_label = ? ORDER BY net_salary DESC LIMIT 5
            ");
            ...
            $stmtOTRep = $pdo->prepare("
                ...
                FROM v_payroll_summary WHERE period_label = ? AND ot_pay > 0 ORDER BY ot_pay DESC
            ");
            $stmtOTRep->execute([$period]);
            $overtime = $stmtOTRep->fetchAll(PDO::FETCH_ASSOC);

            // Bonus Breakdown (FIXED: Joined the departments table to avoid Error 1054)
