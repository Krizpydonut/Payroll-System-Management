...
CREATE PROCEDURE sp_compute_payroll(
  ...
  IN  p_month_year    VARCHAR(7), -- CHANGED: Using month_year
...
BEGIN
  ...
  IF v_emp_type = 'monthly' THEN
    -- Monthly logic: Assumes full month rate / 26 avg working days
  ...
  -- CHANGED: Query bonuses based on month_year
  SELECT COALESCE(SUM(amount), 0) INTO v_total_bonuses
    FROM bonuses WHERE employee_id = p_employee_id AND month_year = p_month_year;

  -- CHANGED: Query deductions based on month_year
  SELECT COALESCE(SUM(amount), 0) INTO v_total_ded
    FROM deductions WHERE employee_id = p_employee_id AND month_year = p_month_year;

  SET v_gross = ROUND(v_basic_pay + v_ot_pay + v_total_bonuses, 2);
  SET v_net   = ROUND(v_gross - v_total_ded, 2);

  -- Upsert payroll row (Using month_year)
  INSERT INTO payroll
    (employee_id, month_year, period_start, period_end,
     total_days, total_hours, total_ot_hours,
     basic_pay, ot_pay, total_bonuses, gross_salary,
     total_deductions, net_salary, status)
  VALUES
    (p_employee_id, p_month_year, p_period_start, p_period_end,
...
CREATE PROCEDURE sp_run_payroll_batch(
  IN p_period_start DATE,
  IN p_period_end   DATE,
  IN p_month_year   VARCHAR(7) -- CHANGED
)
BEGIN
  ...
  read_loop: LOOP
    ...
    CALL sp_compute_payroll(v_emp_id, p_period_start, p_period_end,
                            p_month_year, v_pay_id, v_net);
  END LOOP;
  CLOSE cur;

  SELECT CONCAT('Batch payroll complete for month: ', p_month_year) AS result;
END$$
...
BEGIN
  ...
  DECLARE v_month_year    VARCHAR(7); -- CHANGED

  -- Fetch basic payroll info
  SELECT employee_id, month_year INTO v_employee_id, v_month_year
  ...
  -- Compile Bonuses (Using month_year)
  ...
  WHERE b.employee_id = v_employee_id AND b.month_year = v_month_year;

  -- Compile Deductions (Using month_year)
  ...
  WHERE d.employee_id = v_employee_id AND d.month_year = v_month_year;

  -- Build Full Payslip Object
...
-- ==========================================
-- VIEWS (Updated for Monthly Groupings)
-- ==========================================
...
SELECT
  ...
  pr.month_year, pr.period_start, pr.period_end, -- CHANGED
...
SELECT
  month_year, COUNT(DISTINCT employee_id) AS employee_count, -- CHANGED
...
FROM payroll GROUP BY month_year, status;

CREATE OR REPLACE VIEW v_deduction_breakdown AS
SELECT
  d.month_year, e.employee_code, -- CHANGED
  CONCAT(e.first_name,' ',e.last_name) AS full_name,
...
GROUP BY d.month_year, d.employee_id, d.deduction_type_id;

CREATE OR REPLACE VIEW v_bonus_breakdown AS
SELECT
  b.month_year, e.employee_code, -- CHANGED
  CONCAT(e.first_name,' ',e.last_name) AS full_name,
...
GROUP BY b.month_year, b.employee_id, b.bonus_type_id;